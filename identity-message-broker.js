...
module.exports = function(RED) {
  ...
  function MessageBrokerNode(n) {
    ...
    // Oauth
    this.oauthUrl = n.oauthUrl;
    this.oidcScope = n.oidcScope;
    ...
    if (this.credentials) {
      ...
      this.oauthUsername = this.credentials.oauthUsername;
      this.oauthPassword = this.credentials.oauthPassword;
    }
    ...
    if (!this.cleansession && !this.clientid) {
      this.cleansession = true;
      this.warn(RED._('imbroker.errors.nonclean-missingclientid'));
    }
      ...
      if (!node.connected && !node.connecting) {
        ...
        try {
          ...
          node.client.on('connect', function() {
            ...
            node.connected = true;
            node.log(
              RED._('imbroker.state.connected', {
                broker:
          ...
          node.client.on('close', () => {
            let reqBody = `client_id=${this.clientId}&client_secret=${
              this.clientSecret
            }&grant_type=password&username=${this.oauthUsername}&password=${
              this.oauthPassword
            ...
            const oauthRequest = async () => {
              ...
              node.client.options.username = json.id_token;
              node.client.options.password = json.access_token;

              console.log(node.client.options)

              if (node.connected) {
                node.connected = false;
                node.log(
                  RED._('imbroker.state.disconnected', {
                    broker:
                ...
                node.log(
                  RED._('imbroker.state.connect-failed', {
                    broker:
                      (node.clientid ? node.clientid + '@' : '') +
                      node.brokerurl
                  })
  ...
  RED.nodes.registerType('identity-message-broker', MessageBrokerNode, {
    credentials: {
      user: { type: 'text' },
  ...
  function MessageBrokerInNode(n) {
    RED.nodes.createNode(this, n);
    this.topic = n.topic;
    ...
    if (
      ...
      return this.warn(RED._('imbroker.errors.invalid-topic'));
    }
    var node = this;
    if (this.brokerConn) {
      this.status({
      ...
      if (this.topic) {
        ...
        this.error(RED._('imbroker.errors.not-defined'));
      }
      this.on('close', function(done) {
        if (node.brokerConn) {
      ...
      this.error(RED._('imbroker.errors.missing-config'));
    }
  }
  RED.nodes.registerType('imbroker in', MessageBrokerInNode);

  function MessageBrokerOutNode(n) {
    RED.nodes.createNode(this, n);
    this.topic = n.topic;
    this.qos = n.qos || null;
      ...
      this.on('input', function(msg) {
        ...
        if (msg.hasOwnProperty('payload')) {
          if (
            ...
            node.warn(RED._('imbroker.errors.invalid-topic'));
          }
        }
      });
      if (this.brokerConn.connected) {
  ...
  RED.nodes.registerType('imbroker out', MessageBrokerOutNode);
};
